...
const EditProductPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append remaining existing images as URLs or IDs if backend supports it
        ...
        >
          <form onSubmit={handleSubmit} encType="multipart/form-data" className="space-y-5">
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="flex gap-3 flex-wrap mb-4">
                {existingImages.map((url, idx) => (
                  <div key={idx} className="relative group">
                    <img src={url} alt={`Existing ${idx}`} className="w-20 h-20 object-cover rounded-lg border-2 border-gray-600 group-hover:border-indigo-400 transition-colors" />
                    <button
                      type="button"
              ...
              <label className="block text-sm font-medium text-gray-300 mb-2">Upload New Images (will be added)</label>
              <div className="relative">
            ...
            {previews.length > 0 && (
              <motion.div variants={itemVariants} className="flex gap-3 flex-wrap">
                {previews.map((url, idx) => (
                  <div key={idx} className="relative group">
                    <img src={url} alt={`Preview ${idx}`} className="w-20 h-20 object-cover rounded-lg border-2 border-indigo-500" />
                    <button
                      type="button"
